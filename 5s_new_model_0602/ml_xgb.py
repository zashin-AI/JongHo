...
def TensorBoardCallback():
    writer = SummaryWriter(log_dir='C:/study/graph/'+ str_time.strftime("%Y%m%d-%H%M%S"))
    def callback(env):
...
model = xgb.train(params, dtrain, evals=[(dtrain, 'train'), (dtest, 'test')], callbacks=[TensorBoardCallback()])

# 가중치 저장
pickle.dump(
    model,
    open(
        'C:/nmb/nmb_data/h5/5s/xgb/project_xgb_default.data', 'wb')
    )
...
# 평가 지표 정의
acc_score = accuracy_score(dtest, y_pred)
log_loss = log_loss(dtest, y_pred)
